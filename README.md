...
## ✨ Реалізовані функції (станом на v2.11.0)

* **(UI/UX) Зміна логіки карток-проєетів v2.11.0 (29.10.2025**)
    * При створені нової книги з'іились ще 2 поля, тепер можно обрати жанр та обкладенку.
    * На картці книги тепер є кольоровий бокс з назвою жанра (для кожного жанра свій колір).
    * Додана кнопка редагування проекта (змінити назву, жанр та обкладенку).
    * Додано на вибір 6 обкладенок в жанрі Фентазі.
    * Трохи перероблен зовнішній вигляд карток.
* **(UI/UX) Зміна загального вигляду додатка v2.10.2 (28.10.2025)**
    * Змінено загальну кольорову схему
    * Додано можливості для зміни зовнішнього вигдляду карток та розширення їх фугкціоналу
    *Додано іконку-логоти перед назвою додатка в хедер
* **Створення головнога екрану з "Колекціями" картки проєктів v2.9.0 (26.10.2025)**
    * (UI/UX) Етап 2: Екран 'Колекція':
        * Повністю перероблено екран списку проєктів (колишній `projects-container`).
        * Замість простого списку, тепер відображається адаптивна сітка (`display: grid`) з картками проєктів.
        * **(CSS)** `style.css` оновлено новими класами `.project-card`, `.project-card-header`, `.project-card-stats` тощо.
        * **(JS)** `src/modules/projects.js`: `renderProjectsList` оновлено для генерації нової HTML-структури карток.
        * Картки тепер відображають **прогрес-бар** (відношення слів до `wordGoal`) та поле **"Жанр"** (яке ми додамо до БД наступним кроком).
    * (UX/UI)(v2.830) (25.10.2025) Покращено ефект редагування імені:
        * Замінено емодзі `✏️` на векторну іконку `assets/edit-icon.svg`.
        * Додано нативну підказку "Змінити ім'я" при наведенні.
        * Замінено ефект зміни кольору на легке збільшення (`transform: scale`) та появу фону на контейнері.
        * Клікабельною стала вся область (ім'я + іконка).
...
* **Архітектурний Рефакторинг (v2.6.0)**
...
## The code is entirely written in the Google's model gemini-2.5-pro    